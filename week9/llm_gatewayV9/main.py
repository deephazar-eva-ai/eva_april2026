...
def _backoff_for(err: Exception, has_model_override: bool = False):
    ...
    if status == 408 or "timeout" in msg or "Timeout" in type(err).__name__: return 10, "timeout"
...
async def chat(req: ChatRequest):
    ...
    while time.time() < deadline:
        ...
        if name is None:
            # Check if any candidate is blocked purely due to cooldown.
            cooldowns = []
            for c in candidates:
                c_atts = [a for a in atts if a["provider"] == c]
                if c_atts and "cooldown" in c_atts[0]["reason"]:
                    # Do not rely on snapshot rounding to 0, if reason says cooldown, we wait at least a tiny bit.
                    cd = router.state[c].snapshot(LIMITS[c])["cooldown_remaining"]
                    cooldowns.append(max(cd, 0.1))
            
            if cooldowns and time.time() < deadline:
                wait_s = min(cooldowns)
                await _asyncio.sleep(min(wait_s + 0.05, 5.0, max(0, deadline - time.time())))
                continue
        ...
        try:
            ...
            if getattr(e, "status", None) == 400:
                # Groq can return 400 for model-side tool call generation failures (tool_use_failed)
                # Failover instead of aborting so another provider can try.
                pass
            if not candidates:
                if getattr(e, "status", None) == 400:
                    raise HTTPException(400, f"{name} bad request: {e}")
