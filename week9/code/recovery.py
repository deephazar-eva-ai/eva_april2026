...
def handle_critic_verdict(nid: str, result, graph, recovered_branches: dict,
    ...
    if target_nid and not recovered_branches.get(target_nid):
        ...
        # have already completed successfully so the recovery Planner
        # can wire them by id instead of re-emitting work already done.
        ...
        print(f"  ↪ critic-fail recovery: planner node {rec_nid} for {target_nid}"
              + (f"; reusing {len(prior_complete)} prior result(s): {', '.join(prior_complete)}"
                 if prior_complete else ""))
    elif target_nid:
