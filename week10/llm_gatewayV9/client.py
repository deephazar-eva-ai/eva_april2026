...
class LLM:
    ...
    def chat(self, prompt: str = None, *,
        ...
        try:
            ...
            print(f"[Gateway Error {r.status_code}] {r.text}", file=sys.stderr)
            raise e
        return r.json()
    ...
    def vision(self, prompt: str, image_data_url: str,
               ...
               agent: Optional[str] = None,
               session: Optional[str] = None) -> dict:
        """Call the /v1/vision endpoint with a base64 image data URL."""
        body = {
            "prompt": prompt,
