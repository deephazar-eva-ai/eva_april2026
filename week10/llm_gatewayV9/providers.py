...
class GeminiProvider(BaseProvider):
    ...
    def _translate_tools(self, tools):
        ...
        for t in tools:
            d = t if isinstance(t, dict) else t.model_dump()
            params = d.get("input_schema") or {"type": "object", "properties": {}}
            decls.append({
                "name": d["name"],
                "description": d.get("description", ""),
                "parameters": _gemini_clean_schema(params),
            })
        ...
        for m in messages:
            ...
            if r == "assistant":
                ...
                for tc in (m.get("tool_calls") or []):
                    # Always provide a thought_signature if missing to bypass the 400 error
                    meta = tc.get("provider_meta") or {}
                    sig = meta.get("thoughtSignature") or meta.get("thought_signature") or "skip_thought_signature_validator"
