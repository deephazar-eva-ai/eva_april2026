...
# (Dynamic MCP schema resolution happens inside mcp_runner.py)
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "computer":
        ...
        from schemas import DesktopOutput
        try:
            output_obj = DesktopOutput(**parsed).model_dump()
        except Exception:
            output_obj = parsed
    ...
    if skill.tools_allowed:
        # Multi-turn tool-use loop. mcp_runner connects to the MCP servers,
        # aggregates tools, dispatches each tool_call the model emits,
