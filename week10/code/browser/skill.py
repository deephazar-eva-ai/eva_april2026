...
_GATEWAY_BLOCK_MARKERS = (
    ...
    ("captcha",                "Please solve the challenge below to continue"),
    ("hcaptcha",               'class="h-captcha"'),
    ("hcaptcha",               "data-hcaptcha-widget-id"),
    ("recaptcha",              'class="g-recaptcha"'),
    ("recaptcha",              "g-recaptcha-response"),
    # Google CAPTCHA
    ("captcha",                "unusual traffic from your computer network"),
    ("captcha",                "why did this happen?"),
...
class BrowserSkill:
    ...
    def __init__(self, *, gateway_url: str = "http://localhost:8109",
                 ...
                 max_steps_a11y: int = 4,
                 max_steps_vision: int = 4,
    ...
    async def _evaluate_extract(self, client: V9Client, content: str, goal: str) -> bool:
        """Intelligently check if the static extraction meets the goal, escalating if not."""
        if len(content) < 200:
            return False
        ...
        prompt = (
            f"You are a web extraction evaluator. The user asked the browser agent to achieve this goal: '{goal}'.\n"
            f"The static HTML extraction layer returned the following text content:\n"
            f"---\n{content[:4000]}\n---\n"
            f"Is this text sufficient to satisfy the user's goal? If the text is just boilerplate, 'Please enable JavaScript' warnings, "
            f"or irrelevant fallback text instead of the actual required content, you must return sufficient=false."
        )
        schema = {
            "type": "object",
        ...
        try:
            res = await client.chat(prompt, schema=schema, schema_name="eval")
            return bool(res.parsed and res.parsed.get("sufficient", False))
        except Exception:
            return True
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        ...
        if html:
            ...
            if block:
                raise RuntimeError(f"gateway_blocked: {block} marker on {final_url}")
            content = _extract(html)
            if await self._evaluate_extract(client, content, goal):
        ...
        if getattr(a11y_result, "gateway_blocked", False):
            raise RuntimeError(a11y_result.note or "gateway_blocked after JS render")
        ...
        if getattr(vis_result, "gateway_blocked", False):
            raise RuntimeError(vis_result.note or "gateway_blocked after JS render")
        ...
        async with async_playwright() as p:
            ...
            try:
                try:
                    ...
                    cfg = DriverConfig(
                        goal=goal, max_steps=max_steps, max_failures=5,
                    ...
                    if not downloads and len(ctx.pages) > 1:
                        page = ctx.pages[-1]
                        try:
                        ...
                        try:
                            ...
                            if path:
                                ...
                                with open(path, "rb") as f:
                                    ...
                                    result.extracted = "\n".join(text)
                                    result.final_url = d.url
                        except Exception as e:
                            pass

                    if not result.extracted:
                        try:
                            result.extracted = await page.evaluate("document.body.innerText")
                    ...
                    return DriverResult(success=False, note=f"playwright/driver error: {type(e).__name__}: {e}")
            finally:
                await browser.close()
