...
class BaseDriver:
    ...
    async def step(self, turn: int) -> tuple[bool, bool, str]:
        try:
            ...
            rec = StepRecord(turn, "", [],
                             f"error: failed to read page state: {type(e).__name__}: {e}",
                             "", "", 0, 0, 0)
            self.steps.append(rec)
            await asyncio.sleep(2.0)
            return False, False, "page state error"

        try:
            parsed, result = await self._decide(snap, turn)
        except Exception as e:
            rec = StepRecord(turn, "", [],
                             f"error: gateway call failed: {type(e).__name__}: {e}",
                             "", "", 0, 0, 0)
            self.steps.append(rec)
            return False, False, "gateway error"
