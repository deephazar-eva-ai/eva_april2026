...
# ── Desktop Automation Workarounds ────────────────────────────────────────

@mcp.tool()
def launch_desktop_app(command: str) -> dict:
    """Launch a desktop application in the background (non-blocking). Example: launch_desktop_app("gnome-calculator")"""
    import subprocess
    import shlex
    try:
        args = shlex.split(command)
        # Using start_new_session=True to fully detach the process from the MCP server
