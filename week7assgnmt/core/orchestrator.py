            ...
            for it in range(
                ...
                for i, g in enumerate(obs.goals):
                    ...
                    if not g.done and g.artifact_index is not None:
                        ...
                        elif hit.kind == "fact" and hit.source != "user_query" and hit.source != "observation":
                            print(f"                  hint=search_knowledge")
                            if "(Hint: use search_knowledge" not in g.description:
                                g.description += " (Hint: use search_knowledge to query indexed chunks)"
                ...
                for goal in ready_goals:
                    ...
                    used_tools = {
                        event.get("tool") for event in history
                        if event.get("kind") == "action" and event.get("goal_id") == goal.goal_id
                    }
                    goal_tools = [t for t in tools if t.get("function", {}).get("name") not in used_tools]

                    # Generic Tool Routing: Only expose web tools if the goal explicitly requests the web
                    is_web_goal = any(keyword in goal.description.lower() for keyword in ["web", "internet", "online", "browser", "fetch"])
                    if not is_web_goal:
                        goal_tools = [
                            t for t in goal_tools 
                            if t.get("function", {}).get("name") not in ["web_search", "fetch_url"]
                        ]
                ...
                tasks = []
                for goal, tc in all_out_tool_calls:
                    used_tools = {
                        event.get("tool") for event in history
                        if event.get("kind") == "action" and event.get("goal_id") == goal.goal_id
                    }
                    
                    if tc.name in used_tools:
                        async def reject_reused_tool(name):
                            return ValueError(f"Tool {name} was already used for this goal. You cannot use it again. Please synthesize an answer from the retrieved context.")
                        tasks.append(reject_reused_tool(tc.name))
                        continue

                    is_web_goal = any(keyword in goal.description.lower() for keyword in ["web", "internet", "online", "browser", "fetch"])
                    
                    if not is_web_goal and tc.name in ["web_search", "fetch_url"]:
                        async def reject_tool(name):
                            return ValueError(f"Tool {name} is not permitted for this goal. Use search_knowledge instead.")
                        tasks.append(reject_tool(tc.name))
                    else:
                        tasks.append(self.action.execute(session, tc))
