...
async def chat(req: ChatRequest):
    ...
    # We do NOT set explicit_override = True here, because the pin is just a 
    # preference and failover should still happen if it's unavailable.
    pinned_agent = None
    ...
    if req.auto_route and not req.provider and not pinned_agent:
        ...
        if req.provider:
            ...
            if pinned_agent:
                candidates = [pinned_agent] + [c for c in candidates if c != pinned_agent]
    ...
    # Wait for the pinned agent if it is blocked by rate limits (RPM or cooldown),
    # rather than failing over immediately to fallbacks that might lack tools.
    if pinned_agent:
        c = pinned_agent
        import asyncio as _a
        while True:
            ok, why = router.state[c].can_use(LIMITS[c], est)
            if ok:
                break
                
            wait = 0.0
            if "unavailable" in why:
                wait = router.state[c].unavailable_until - time.time()
            elif "cooldown" in why:
                wait = router.state[c].snapshot(LIMITS[c])["cooldown_remaining"]
            elif "RPM" in why or "quota" in why:
                now = time.time()
                window = [t for t, _ in router.state[c].tokens_minute if now - t < 60]
                if window:
            ...
            wait = min(wait, 60)
            print(f"[gateway] {req.session[:8] if req.session else 'req'} {c} blocked by {why}, waiting {wait:.1f}s")
            await _a.sleep(wait + 0.5)
    ...
    for _ in range(len(candidates) + 1):
        ...
        try:
            ...
            # The ProviderError may declare itself non-retryable (e.g. 401 Auth Error)
            # but that just means we shouldn't retry the SAME provider. We still want
            # to fail over to the NEXT provider unless the caller explicitly pinned one.
