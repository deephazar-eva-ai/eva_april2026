...
async def run_with_tools(*, prompt: str, tools_payload: list[dict],
    ...
    try:
            ...
            async with ClientSession(read, write) as mcp:
                ...
                for _ in range(MAX_TOOL_HOPS + 1):
                    try:
                        ...
                        return {"error": f"LLM error: {e}", "text": ""}
        ...
        if type(e).__name__ == "ExceptionGroup" or "ExceptionGroup" in type(e).__name__:
            err_str += " [" + ", ".join([f"{type(ex).__name__}: {ex}" for ex in getattr(e, "exceptions", [])]) + "]"
        return {"error": err_str, "text": ""}
