...
class Executor:
    ...
    async def run(self, query: str, *, session_id: str | None = None,
                  resume: bool = False, on_event=None) -> str:
        async def emit(msg: str, **kwargs):
            print(msg)
            if on_event:
                if asyncio.iscoroutinefunction(on_event):
                    await on_event({"message": msg, **kwargs})
                else:
                    on_event({"message": msg, **kwargs})
        ...
        if memory_hits:
            await emit(f"[memory.read] {len(memory_hits)} hit(s) visible to every skill this run")
        try:
            memory_svc.remember(query, source="user_query", run_id=sid)
        except Exception as e:
            await emit(f"[memory.remember] skipped: {e!r}")
        ...
        while True:
            ...
            if executed_count + len(ready) > MAX_NODES:
                await emit(f"[flow] node cap {MAX_NODES} hit at {executed_count}; stopping")
                break
            ...
            for nid, result, prompt in outcomes:
                ...
                await emit(f"[{nid}] {graph.g.nodes[nid]['skill']:18s} "
                      f"{graph.g.nodes[nid]['status']:8s} "
                      f"({result.elapsed_s:.1f}s)"
                      + (f"  err={result.error[:80]}" if result.error else ""),
                      event_type="node_status", node_id=nid, skill=graph.g.nodes[nid]['skill'], status=graph.g.nodes[nid]['status'], elapsed_s=result.elapsed_s, iteration=executed_count)
                ...
                if result.success:
                    ...
                    if decision.action == "skip":
                        await emit(f"  ↪ {nid} failed ({decision.reason}, "
                    ...
                    await emit(f"  ↪ recovery ({decision.reason}): planner node "
        ...
        if critic_fail_cap_hit:
            ...
            await emit(f"\n[flow] WARNING: critic-fail cap hit on "
                  ...
                  f"output too.")
        await emit(f"\n{'═' * 78}\nFINAL: {formatter_answer or ''}\n{'═' * 78}\n", event_type="final", final_answer=formatter_answer or "")
